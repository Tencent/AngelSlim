...
#     places angelslim/.../vllm_calibrate_utils.py at <vllm>/tools/.
...
CALIB_UTILS_SRC="${ANGELSLIM_ROOT}/angelslim/compressor/quant/core/vllm_calibrate_utils.py"
...
do_install() {
    ...
    local utils_dst="${utils_dst_dir}/vllm_calibrate_utils.py"

    # ---- sanity-check sources ----
    for f in "${PATCH_ENVS_SRC}" "${PATCH_FUSED_MOE_SRC}" "${CALIB_UTILS_SRC}"; do
    ...
    cp -p "${CALIB_UTILS_SRC}" "${utils_dst}"
    log "Installed ${utils_dst}"
...
do_uninstall() {
    ...
    local envs_dst="${vllm_dir}/envs.py"
    local fused_moe_dst="${vllm_dir}/model_executor/layers/fused_moe/fused_moe.py"
    local utils_dst="${vllm_dir}/tools/vllm_calibrate_utils.py"
...
do_check() {
    ...
    if [[ -f "${utils_dst}" ]]; then
        log "  [OK]   ${utils_dst} exists"
    else
        err  "  [FAIL] ${utils_dst} does NOT exist"
        ok=0
    fi
