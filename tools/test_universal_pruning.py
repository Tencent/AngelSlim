...
# Ensure the root directory is in the search path to find the token_compressor package
# Assuming this script is in 'tools/', the project root is one level up
...
DEFAULT_IMAGE_PATH = "https://github.com/Tencent/AngelSlim/raw/main/docs/source/assets/logos/angelslim_logo.png"
MAX_NEW_TOKENS = 30
...
def load_image(image_source: str) -> Image.Image:
    ...
    Args:
        image_source (str): The source identifier, can be a local path or an HTTP(S) URL.
...
def main():
    parser = argparse.ArgumentParser(description="Tailored Smoke Test for DART and Universal Architecture")
    parser.add_argument("--config", type=str, required=True, help="Path to the strategy YAML (e.g., configs/qwen2_5_vl/pruning/dart_r0.75.yaml)")
    parser.add_argument("--model_path", type=str, default="Qwen/Qwen2.5-VL-3B-Instruct", help="HuggingFace model path or local path")
    ...
    try:
        ...
        model = AutoModelForMultimodalLM.from_pretrained(
            ...
            attn_implementation="sdpa", # Hooks and attention-based pruning require eager implementation
            device_map="auto",
    ...
    try:
        ...
        inputs = processor(text=[text_prompt], images=[image], padding=True, return_tensors="pt").to(model.device)
        print(f"✅ Multimodal inputs prepared.")
    except Exception as e:
    ...
    adapter = None
    try:
        print(f"🛠️  Applying UniversalPruningAdapter using steps from YAML...")
