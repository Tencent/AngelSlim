...
def validate_vllm_calibration_dp_args(parser, args):
    ...
    if args.dp_num_nodes != 1:
        parser.error(
            "Ray actor-managed calibration DP launches all replicas from one driver process; "
            "please keep --dp-num-nodes=1"
        )
    if args.dp_node_rank != 0:
        parser.error(
            "Ray actor-managed calibration DP launches all replicas from one driver process; "
            "please keep --dp-node-rank=0"
        )
    return args
...
def _run_ray_actor_calibration_dp(args, worker_fn: Callable[[Any], Any]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        result_refs = [
            actor.run.remote(args_dict, dp_rank)
        ...
        _merge_dp_payloads(args.output_dir, results)
        ...
        if args.search_kv_scale and args.kv_granularity != "none":
            ...
            if not os.path.exists(activation_stats_path):
                raise FileNotFoundError(
                    "Merged activation_stats.json is required for DP KV scale search: "
                    f"{activation_stats_path}"
                )
            ...
            args_dict = vars(copy.deepcopy(args))
            kv_refs = [
                actor.run_kv_search.remote(args_dict, dp_rank, activation_stats_path)
                for dp_rank, actor in enumerate(actors)
            ]
            ready, pending = ray.wait(
                kv_refs,
            ...
            _merge_dp_kv_search_payloads(args.output_dir, kv_results, args.kv_granularity)
...
class CalibrationReplica:
    ...
    def _prepare_args(
        ...
        os.environ["ANGELSLIM_CALIBRATION_DP_RANK"] = str(dp_rank)
        os.environ["ANGELSLIM_CALIBRATION_DP_SIZE"] = str(args.dp_size)
    ...
    def run_kv_search(
        ...
        args.kv_search_activation_stats_path = activation_stats_path
...
def _merge_dp_payloads(output_dir: str, results: list[dict[str, Any]]) -> None:
    """Merge calibration statistics from all DP ranks."""
    print("\n" + "=" * 80)
    print("Merging DP calibration statistics...")
    print("=" * 80)
    ...
    for filename in _MINMAX_FILES:
        ...
        for key, rank_data in all_entries.items():
            ...
            if not isinstance(first_stats, dict) or "min" not in first_stats or "max" not in first_stats:
                print(f"  Key {key}: Invalid stats format, skipping")
                continue

            # Check if min/max are scalars or lists
            first_min = first_stats["min"]
            first_max = first_stats["max"]
    ...
    print("\n" + "=" * 80)
    print("DP calibration merge completed!")
    print("=" * 80)
...
def _select_best_multiplier_from_profiles(profile_entries: list[dict[str, Any]]) -> tuple[float, float]:
...
def _merge_dp_kv_search_payloads(
    ...
    print("\n" + "=" * 80)
    print("Merging DP KV-cache scale search profiles...")
    print("=" * 80)
    ...
    if kv_granularity == "per-tensor":
        ...
        for rank, payload in enumerate(payloads):
            ...
            for stats_key, profile in profiles.items():
                all_profiles.setdefault(stats_key, []).append(profile)

        kv_multipliers = {}
        for stats_key, entries in all_profiles.items():
        ...
        all_profiles: dict[str, list[list[dict[str, Any]]]] = {}
        for rank, payload in enumerate(payloads):
            profiles = payload.get(profile_key, {})
            if not profiles:
                print(f"  Rank {rank}: no {profile_key} found")
            ...
            for stats_key, head_profiles in profiles.items():
                all_profiles.setdefault(stats_key, []).append(head_profiles)

        kv_multipliers_perhead = {}
        for stats_key, rank_head_profiles in all_profiles.items():
