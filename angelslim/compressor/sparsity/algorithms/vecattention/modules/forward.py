...
# Ensure VecAttention's custom vllm_flash_attn is importable. The kernel is an
# EXTERNAL optional dependency — a compiled vLLM-flash-attention fork — NOT
# vendored in this tree (see algorithms/vecattention/NOTICE). A normal
# ``pip install`` of that fork registers ``vllm_flash_attn`` in site-packages, so
# the bare import below just works. As a belt-and-suspenders fallback, IF a
# contributor keeps a local build of the fork under the legacy
# ``vecattention/ops/vllm-flash-attention/`` path (it is .gitignored, never
# tracked), we add that dir to sys.path — APPENDED (not prepended), so a properly
# pip-installed ``vllm_flash_attn`` is never shadowed by the local fork dir.
# From this file: modules -> vecattention -> algorithms ->
# sparsity, then vecattention/ops/vllm-flash-attention.
_SPARSITY_DIR = os.path.dirname(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
)
_VLLM_FA_DIR = os.path.join(_SPARSITY_DIR, "vecattention", "ops", "vllm-flash-attention")
if os.path.isdir(_VLLM_FA_DIR) and _VLLM_FA_DIR not in sys.path:
    sys.path.append(_VLLM_FA_DIR)
...
try:
    ...
    raise ImportError(
        ...
        "It is an external optional dependency (a compiled vLLM-flash-attention "
        "fork), not vendored in this repo. Install it into your environment:\n"
        "  pip install vllm_flash_attn   # or build the fork: pip install -e .\n"
        "Without it, vecattention runs the pure-torch reference "
        "(set compression.sparsity.allow_pseudo_sparse=true)."
    )
...
def qwen_vl_attn_forward(
    ...
    **kwargs,
    ...
    # Sparse prefill ONLY on a true first-fill: q_len>1, q_len>=block, AND no
    # cache prefix (k_len==q_len). A cached 2nd turn / chunked prefill has
    # k_len>q_len, which mis-anchors the block-causal selection — route it to
    # the model's configured attention via the decode fallback below.
    if q_len > 1 and q_len >= config.block_size_q and key_states.shape[2] == q_len:
