...
#
# Vendored into AngelSlim from MInference (https://github.com/microsoft/MInference)
# at revision a4eb395: minference/modules/flexprefill.py. MIT, (c) Microsoft.
# Reformatted to this repo's style (black/isort); the Triton kernels and the
# gamma/tau block-selection keep their exact upstream behavior. Five small local
# modifications (standalone ImportError, dropped __main__ smoke block, block-size
# divisor / device-move / assert-message fixes) are itemized in kernels/NOTICE.
# Public entry: `flex_prefill_attention(q, k, v, gamma, tau, ..., block_size)`
# with q/k/v shaped (batch, seq, num_heads, head_dim); head_dim must be in
# {16, 32, 64, 128} (the FlexPrefill algorithm gates head_dim 256 to the torch
# reference, mirroring minference).
...
except ImportError as _e:  # vendoring: no relative fallback (flash_attn is a hard dep here)
    raise ImportError(
        "flexprefill vendored kernel requires flash_attn (the short-seq path and "
        "the upstream Triton fallback both route through flash_attn_func). Install "
        "it with:  pip install flash-attn --no-build-isolation"
    ) from _e
...
def torch_block_wise_attention(
    ...
    num_block = math.ceil(grid_offset / block_size) + math.ceil((n - grid_offset) / block_size)
    ...
    act_blocks_per_row = torch.tril(mask).sum(-1)  # noqa: F841
...
def block_wise_decode_attention_kernel(
    ...
    # flash attention
    for i in range(0, NUM_BLOCK):  # noqa: B007
...
def triton_block_wise_decode_attention(
    ...
    Args:
        ...
        block_idx (torch.Tensor): Index of activated blocks, shape [batch_size, num_heads, activated_block_num]  # noqa: E501
...
def triton_column_count_cumsum(x: torch.Tensor, num_columns: int) -> torch.Tensor:
    ...
    Args:
        x (torch.Tensor): block index in a flatten 2d grid, shape [batch_size, num_heads, activated_block_num]  # noqa: E501
        num_colums (int): number of columns in the grid

    Returns:
        torch.Tensor: cumsum of columns num in each row, shape [batch_size, num_heads, num_rows + 1 ]  # noqa: E501
            For example, in a 4x4 block grid, activated blocks have index [0, 5, 8, 9, 13, 14], number of blocks in each row is [1, 1, 2, 2],  # noqa: E501
...
def block_wise_prefill_attention_kernel(
    ...
    # get column block index ptr
    block_idx_ptr = block_idx_ptr + pid_b * stride_bb + pid_h * stride_bh + bin_start * stride_bt
    # init qkv ptrs
    q_ptrs = tl.make_block_ptr(
        base=q_ptr + pid_b * stride_qb + pid_h * stride_qh,
    ...
    acc_o = tl.full((BLOCK_SIZE_Q, HEAD_DIM), 0, dtype=tl.float32)
    # flash attention
    for i in tl.range(0, num_active_block):  # noqa: B007
        # get current block start index
...
def triton_block_wise_prefill_attention(
    ...
    Args:
        ...
        block_idx (torch.Tensor): Index of activated blocks, shape [batch_size, num_heads, activated_block_num], which is the index of the flattened block grid.  # noqa: E501
            For example, in a 4x4 block grid, if you want to activate 5 blocks: (0,0), (1,1), (2,0), (3,1), (3,2), the index will be: [0, 5, 8, 13, 14]  # noqa: E501
        block_size (int): Block size, only support 16, 32, 64 and 128.
        grid_offset (int): Move the grid that divides the block to the lower left corner by grid_offset, default to 0.  # noqa: E501
        softmax_scale (Optional[float], optional): Softmax scale. Defaults to 1/math.sqrt(head_dim)
        gqa_interleave (bool): use interleave mode of gqa, default to False.
...
def triton_bnhd_pool(x: torch.Tensor, kernel_size: int, pool_type: str = "avg"):
    # NB: do NOT force x onto the default CUDA device — x is already on its
    # layer's device, and a hardcoded .to("cuda") would corrupt results under
    # accelerate device_map sharding. The kernel launches on x.device below.
    b, n, h, d = x.shape
...
def bhn_sumpool_kernel(
    ...
    y = tl.sum(x, axis=1)
    y_ptr = y_ptr + pid_b * stride_yb + pid_h * BLOCK_SIZE_H * stride_yh + pid_n * stride_yn
...
def transform_veritcal_slash_idx(v_idx, s_idx, num_blocks):
    ...
    # vertical
    v_idx = (
        torch.arange(0, num_blocks, device=v_idx.device)[None, None, :, None] * num_blocks
        + v_idx[:, :, None, :]
    ).view(batch_size, num_heads, -1)
    v_idx[v_idx // num_blocks < v_idx % num_blocks] = 0
    # slash
    s_idx = (range_blocks * num_blocks + range_blocks + s_idx[:, :, None, :] * num_blocks).view(
        batch_size, num_heads, -1
    )
    s_idx[s_idx >= num_blocks * num_blocks] = 0
    # union
    vs_idx = torch.cat((s_idx, v_idx), dim=-1)
    block_idx = [[torch.unique(vs_idx[b, h]) for h in range(num_heads)] for b in range(batch_size)]
    return block_idx
...
def square_root_js_divergence(p: torch.Tensor, q: torch.Tensor):
    m = (p + q) / 2
    return torch.sqrt(0.5 * (p * torch.log(p / m)).sum(-1) + 0.5 * (q * torch.log(q / m)).sum(-1))
...
def get_active_blocks(
    ...
    # score_cover_topk returns a TOKEN-position count; divide by block_size (not a
    # hardcoded 128) to convert to a BLOCK budget — otherwise a block_size != 128
    # (a valid {16,32,64,128} knob) silently scales the vertical/slash budget by
    # block_size/128 and over- or under-sparsifies.
    ...
    vertical_topk = vertical.topk(min(num_vertical_blocks.max().item(), num_blocks), -1).indices
    vertical_topk[
...
def flex_prefill_attention(
    ...
    num_blocks = math.ceil(seq_len / block_size)  # noqa: F841
...
def flexprefill_forward(
    ...
    out = flex_prefill_attention(q, k, v, gamma, tau, min_budget, max_budget, block_size)
