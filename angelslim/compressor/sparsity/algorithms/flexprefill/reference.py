...
"""Pure-torch FlexPrefill reference (pseudo-sparse fallback path + oracle).

This mirrors the *shape* of upstream FlexPrefill's selection — per query-block,
keep the smallest set of (causal) key-blocks whose pooled, softmaxed attention
mass covers ``gamma`` — at boolean-mask granularity, then runs masked softmax
attention. It is the fallback when the fused Triton kernel cannot run (e.g.
head_dim 256 on Qwen3.5) and the numerical oracle the tests compare against. It
is NOT a perf path (materializes the full L×L scores); ``gamma → 1`` recovers
dense attention exactly, which is what the correctness tests assert.
...
def flexprefill_reference_prefill(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    *,
    ...
    the top key-blocks whose cumulative mass first reaches ``gamma``. The chosen
    key-blocks form a block mask, expanded to a full token mask and combined with
    the causal mask for the final softmax. gamma>=1 keeps every causal block
    (== dense).

    ``tau`` (upstream's block-sparse degradation threshold) is accepted for
    signature parity with the kernel path but not used by this reference, which
    always takes the gamma-coverage path — the conservative (more-keys) choice,
    so the reference never drops MORE than the kernel would.
    """
    del tau  # reference uses gamma-coverage only (see docstring)
    B, H, L, D = q.shape
    ...
    # Build a per-(query-block, key-block) keep mask via gamma coverage.
    keep_block = torch.zeros(B, H, nqb, nkb, device=q.device, dtype=torch.bool)
    for qb in range(nqb):
        q0, q1 = qb * block_size, min((qb + 1) * block_size, L)
        # representative query-block score over key tokens: max across the rows.
        ...
        # sort descending, take the prefix whose cumsum first reaches gamma.
        sorted_mass, order = torch.sort(mass, dim=-1, descending=True)
        csum = torch.cumsum(sorted_mass, dim=-1)
        # number of blocks needed: first index where csum >= gamma, +1.
        reached = (csum >= gamma).float()
        # argmax of reached gives the first True; if gamma>=1 none reach -> keep all.
        ...
        # scatter: keep the top-n_keep key-blocks (by sorted order) for this qb.
