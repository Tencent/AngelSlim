...
# The HPC C++ extension (`hpc`) is an OPTIONAL internal kernel package that is
# NOT publicly installable. Importing it at module load would make a bare
# `import angelslim...stem.backends.hpc_impl` (e.g. via the dispatcher) raise
# ModuleNotFoundError for every external user who merely configured
# `backend: hpc`. Instead it is imported lazily through `_require_hpc()`, which
# raises an ACTIONABLE error — consistent with the kernel-missing policy the
# other algorithms use. `hpc` is bound as a module global on first successful
# import so the `hpc.*` references throughout this file resolve normally.
hpc = None


def _require_hpc():
    """Import the optional HPC extension, with an actionable error if absent."""
    global hpc
    if hpc is None:
        try:
            import hpc as _hpc
            ...
            raise ImportError(
                "Stem `backend: hpc` requires the internal `hpc` C++ extension, "
                "which is not installed (and not publicly available). Use "
                "`backend: torch` in your sparsity config, or install the hpc "
                "extension into your environment."
            ) from exc
        hpc = _hpc
    return hpc
...
def stem_forward_hpc(
    ...
    # --- Guard: the optional hpc extension must be importable -------------
    try:
        _require_hpc()
        ...
        if strict_hpc:
            raise
        if layer_idx == 0:
            print(f"[Stem][HPC] {exc}; falling back to torch backend.")
        return _fallback_to_torch(query_states, key_states, value_states, prefill_kwargs)
