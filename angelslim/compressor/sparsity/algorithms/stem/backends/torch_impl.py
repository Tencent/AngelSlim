...
"""PyTorch + Triton implementation of the Stem sparse prefill.

This is the ``backend: torch`` path. It is NOT a CPU / no-Triton pure-PyTorch
reference: the block-importance scoring always runs a Triton kernel (see
:func:`_compute_triton_block_logits`), so a CUDA device with Triton is required.
``allow_pseudo_sparse`` only affects the FINAL attention step — when
``block_sparse_attn`` is absent it expands the top-k mask and runs dense masked
attention instead of the block-sparse kernel; the Triton scoring stage is
...
try:
    ...
    # Do NOT print-and-silently-degrade here. The decision of
    # whether to fall back to pseudo-sparse belongs to the call site, gated by
    # the user's ``allow_pseudo_sparse`` flag. Import failure just records
    # availability; ``stem_forward_torch`` enforces the hard-fail policy.
...
def generate_exact_k_schedule(
    ...
    """
    # Prefer an explicitly-provided per-instance ``keep_ratio``
    # (derived against the actual ``num_hidden_layers``). Only fall back to the
    # hardcoded Qwen3-8B schedule when no ratio was threaded through — and even
    # then guard the index so models with != 38 layers do not IndexError.
    if keep_ratio is None:
        if 0 <= layer_idx < len(_DEFAULT_LAYER_KEEP_RATIOS):
            keep_ratio = _DEFAULT_LAYER_KEEP_RATIOS[layer_idx]
        else:
            # Past the hardcoded table: reuse the steady-state ratio.
            keep_ratio = _DEFAULT_LAYER_KEEP_RATIOS[-1]
...
def _compute_triton_block_logits(
    ...
    # NB: pad/scoring tensors MUST stay on the *input's* device. Hardcoding
    # ``.to("cuda")`` resolves to the current default device (cuda:0) and, under
    # accelerate ``device_map`` layer sharding, silently yanks a cuda:1 layer's
    # Q/K/V to cuda:0 — so block scoring runs against the wrong device and the
    # selected blocks diverge from the single-device result (G1 caught this:
    # sharded keep=1.0 sparse drifted rel~0.15 vs dense while single-device was
    # ~0.01). ``.to(device)`` (== query_states.device) is correct on 1 GPU and
    # under sharding alike.
    if k_pad > 0:
        pad_key_states = F.pad(key_states, (0, 0, 0, k_pad), value=0).to(device)
        value_norm = torch.norm(value_states, p=2, dim=-1, keepdim=True).to(device)
        pad_value_states = F.pad(value_norm, (0, 0, 0, k_pad), value=0).to(device)
    else:
    ...
    if q_pad > 0:
        pad_query_states = F.pad(query_states, (0, 0, 0, q_pad), value=0).to(device)
    else:
...
def stem_forward_torch(
    ...
    # Per-instance keep-ratio derived against the real layer count.
    layer_keep_ratios = config.get("layer_keep_ratios")
    keep_ratio = None
    if isinstance(layer_keep_ratios, (list, tuple)) and 0 <= layer_idx < len(layer_keep_ratios):
        keep_ratio = layer_keep_ratios[layer_idx]
    sched = generate_exact_k_schedule(
        Qb, alpha, layer_idx, query_states.device, num_heads=H, keep_ratio=keep_ratio
    )
    ...
    # Hard-fail when the real kernel is missing unless
    # the user explicitly opted into pseudo-sparse.
    allow_pseudo_sparse = bool(config.get("allow_pseudo_sparse", False))
    if not HAS_BLOCK_SPARSE_KERNEL and not allow_pseudo_sparse:
        from ...._kernel_check import KernelMissingError

        raise KernelMissingError(
            "[sparse:stem] required kernel 'block_sparse_attn' is not "
            "installed.\nInstall hint:\n  pip install block_sparse_attn\n"
            "To run anyway with the slow pseudo-sparse fallback, set "
    ...
    from ...._kernel_check import warn_pseudo_sparse_fallback

    warn_pseudo_sparse_fallback("stem", "block_sparse_attn not installed")
    mask_full = (
