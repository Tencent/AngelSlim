...
class CosyVoice3DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        batch_prompt_speech_tokens = torch.cat(
            [paddingtensor2D(item["prompt_speech_token"].unsqueeze(0), max_length) for item in features]
        )
        batch_text_token_lens = torch.stack(
            [item["text_len"] for item in features]
        )
