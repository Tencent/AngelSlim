...
from .draft import Llama3Eagle3Drafter, CosyVoice3Llama3Eagle3Drafter
...
class CosyVoice3Eagle3Model(nn.Module):
    """
    CosyVoice3 EAGLE3 Model for speculative decoding with improved structure and maintainability
    """
    ...
    def eagle_generate(
        ...
        **kwargs,
        ...
        input_ids = torch.concat(
            [
                self.base_model.model.llm.sos_id.unsqueeze(dim=0).to(text.dtype).to(text.device),
                prompt_text,
                text,
                self.base_model.model.llm.task_token.unsqueeze(dim=0).to(text.dtype).to(text.device),
    ...
    def naive_generate(
        ...
        **kwargs,
        ...
        for step in range(max_decode_steps):  # noqa: B007
            input_id = self.base_model.model.llm.sampling_ids(logits.squeeze(dim=0), out_tokens, ignore_eos=True if step < min_len else False)
            out_tokens.append(input_id)
            input_id = torch.tensor(input_id, device=state.input_ids.device).unsqueeze(0).unsqueeze(0)
