...
from typing import Optional, Callable, List, Generator, Any
...
from .......utils.lazy_imports import torchaudio, whisper, onnxruntime, inflect, librosa, wetext
...
class CosyVoiceFrontEnd:
    ...
    def _extract_spk_embedding(self, prompt_wav):
        speech = load_wav(prompt_wav, 16000)
        feat = torchaudio.compliance.kaldi.fbank(speech,
                           num_mel_bins=80,
                           dither=0,
...
class CosyVoice3LM(torch.nn.Module):
    def __init__(
        ...
        # 4. sampling method
        self.sampling = functools.partial(ras_sampling, 
                                          top_p=0.8,
                                          top_k=25,
                                          win_size=10,
                                          tau_r=0.1)
    ...
    def inference_one_step(
        ...
        inputs_embeds = self.speech_embedding.weight[input_ids.squeeze(0).tolist()].unsqueeze(0)

        out_tokens = []
        if pre_tokens is not None:
            out_tokens.extend(pre_tokens)
        y_pred, hidden_states = self.llm.forward_one_step(inputs_embeds,
                                                    past_key_values=past_key_values,
                                                    position_ids=position_ids,
    ...
    def forward(
        ...
        # prefill
        out_tokens = []
        y_pred, hidden_states = self.llm.forward_one_step(lm_input,
                                                    past_key_values=past_key_values,
                                                    output_hidden_states=True,
                                                    return_hidden_states=True,)
...
class CosyVoice3:
    ...
    def __init__(self, model_dir, generate_audio=False):
        self.config = PretrainedConfig.from_pretrained(os.path.join(model_dir, "CosyVoice-BlankEN"))
        self.config.model_type = "cosyvoice3"
        self.config.txt_tokenizer_path = os.path.join(model_dir, "CosyVoice-BlankEN")
        self.dtype = self.config.torch_dtype
        
        self.model_dir = model_dir
        self.frontend = CosyVoiceFrontEnd(partial(get_qwen_tokenizer,
                                                  token_path=self.config.txt_tokenizer_path,
                                                  skip_special_tokens=True),
                                          partial(mel_spectrogram,
                                                  n_fft=1920,
                                                  num_mels=80,
        ...
        if self.generate_audio:
            ...
            decoder = CausalConditionalCFM(in_channels=240,
                                           n_spks=1,
                                           spk_emb_dim=80,
                                           cfm_params=cfm_params,
                                           estimator=estimator,)
            flow = CausalMaskedDiffWithDiT(input_size=80,
                                           output_size=80,
            ...
            hift = CausalHiFTGenerator(in_channels=80,
                                       base_channels=512,
                                       nb_harmonics=8,
                                       sampling_rate=sample_rate,
                                       nsf_alpha=0.1,
            ...
            flow_path, hift_path = os.path.join(model_dir, "flow.pt"), os.path.join(model_dir, "hift.pt")
