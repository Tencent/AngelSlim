...
def plan_pruning_execution(
    ...
    Logic:
    1. Perform dynamic routing for 'model_related_params' by matching patterns against model path.
    2. Resolve negative indices in 'vit_q_layers' and 'vit_k_layers' using vision depth.
    3. Determine which components need to be wrapped based on requirements and strategies.
    ...
    for stage_key, strategy in strategy_config.strategies.items():
        # Check if the strategy defines parameters that depend on the model path
        if hasattr(strategy, "model_related_params") and strategy.model_related_params:
            for param_name, pattern_map in strategy.model_related_params.items():
                resolved_value = None
                # Iterate through the pattern dictionary (e.g., {"7b": "...", "3b": "..."})
                for pattern, value in pattern_map.items():
                    if pattern.lower() in model_path_str:
                        resolved_value = value
                        break

                # Strict principle: Raise error if a required mapping is missing for the current model
                if resolved_value is None:
                    raise RuntimeError(
                        f"[AngelSlim Error] Strategy '{strategy.strategy}' requires dynamic parameter '{param_name}', "
                        f"but no matching pattern was found in model path: '{model_path_str}'. "
                        f"Available patterns: {list(pattern_map.keys())}"
                    )

                # Inject the resolved value into the active parameters dictionary
                strategy.params[param_name] = resolved_value
                print(f"[ConfigUtils] Resolved dynamic param '{param_name}' to '{resolved_value}' for model '{model_path_str}'")
    ...
    if vision_cfg is not None:
        ...
        if isinstance(vision_cfg, dict):
            vision_depth = vision_cfg.get("depth", None) or vision_cfg.get("num_hidden_layers", None)
        else:
            vision_depth = getattr(vision_cfg, "depth", None) or getattr(vision_cfg, "num_hidden_layers", None)

    def resolve_negative_indices(layers: List[int], depth: int) -> List[int]:
        """Resolves indices like -1 to actual depth-based indices."""
        return [l + depth if l < 0 else l for l in layers]

    requirements = strategy_config.requirements
    if vision_depth > 0:
        requirements.vit_q_layers = resolve_negative_indices(requirements.vit_q_layers, vision_depth)
        requirements.vit_k_layers = resolve_negative_indices(requirements.vit_k_layers, vision_depth)

    # --- 3. Execution Plan Construction ---
    # Resolve required Vision indices (Union of Q and K capture requirements)
    required_vit_indices: Set[int] = set(requirements.vit_q_layers) | set(requirements.vit_k_layers)
    
    # Resolve required LLM indices (Capture requirements + Layers with active pruning)
    required_llm_indices: Set[int] = set(requirements.llm_q_layers) | set(requirements.llm_k_layers)
    ...
    # The Vision Transformer wrapper is required if any internal vision layers need to be hooked
